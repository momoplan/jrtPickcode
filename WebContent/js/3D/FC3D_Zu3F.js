    ...
    function CheckFullZu3F() {
        ...
        var p = invest * 2;
        $('#lab_Zu3F_Selected').html(invest + decodeURI(EncodeUtf8('注，'))+ p.toFixed() +decodeURI(EncodeUtf8('元。')));
        if(invest < 2)  $("#btn_Zu3F_Add").attr('isdisabled', 'true');
        else  $("#btn_Zu3F_Add").attr('isdisabled', 'false');
    }
    ...
    function btn_2_AddClickZu3F() {
    	var invest = GetLotteryInvestNumZu3F();
        if (invest < 2) {
            alert(decodeURI(EncodeUtf8("您选择的不是一注复式票！\n应至少选择 2个号码。请检查您选择的号码。")));
            return false;
        }		
